...
- [x] **No default scene — editor is unusable without external Playground snippet** — Opening the editor shows an empty preview with "Load a Playground snippet to preview the scene". There is no built-in default scene (box + camera + light), so you cannot start wiring logic without first creating a separate Playground snippet. Every session requires leaving the editor to set up a scene. **Expected:** The editor should create a minimal default scene (camera, light, ground, a few meshes) so users can immediately start experimenting with flow graph logic.
